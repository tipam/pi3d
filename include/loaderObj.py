...
#########################################################################################
def loadFileOBJ(self,fileName,texs, xx=0,yy=0,zz=0, rotx=0,roty=0,rotz=0, sx=0,sy=0,sz=0, cx=0,cy=0,cz=0):
    ...
    for g in faces:
        numv[g] -= 1
        numi[g] -= 1
        self.vGroup[g] = create_shape("", self.x+xx, self.y+yy, self.z+zz, self.rotx+rotx,self.roty+roty,self.rotz+rotz, self.sx+sx,self.sy+sy,self.sz+sz, self.cx+cx,self.cy+cy,self.cz+cz)
